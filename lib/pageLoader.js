...
function urlToResourceName(resourceUrl, pageUrl, type) {
    ...
    // Prefijo según tipo de recurso
    let prefix;
    if (type === 'img') {
        // imágenes solo usan host
        prefix = new URL(pageUrl).host.replace(/^www\./, '').replace(/\./g, '-');
    } else {
        // CSS/JS incluyen la ruta de la página
        prefix = urlToFilename(pageUrl);
    }
...
export default async function pageLoader(pageUrl, outputDir = process.cwd()) {
    ...
    $('img').toArray().forEach(img => {
        ...
        if (src && isLocalResource(src, pageUrl)) {
            ...
            const filename = urlToResourceName(src, pageUrl, 'img');
    ...
    $('link[rel="stylesheet"]').toArray().forEach(link => {
        ...
        if (href && isLocalResource(href, pageUrl)) {
            ...
            const filename = urlToResourceName(href, pageUrl, 'css');
    ...
    $('script[src]').toArray().forEach(script => {
        ...
        if (src && isLocalResource(src, pageUrl)) {
            const absoluteUrl = new URL(src, pageUrl).toString();
            const filename = urlToResourceName(src, pageUrl, 'js');
