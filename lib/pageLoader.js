...
function urlToResourceName(resourceUrl, pageUrl) {
    ...
    // Tomar solo el último segmento del path
    const segments = absoluteUrl.pathname.split('/').filter(Boolean);
    const baseName = segments.pop().replace(ext, ''); // nombre real del archivo
    const name = baseName.replace(/[^a-z0-9]/gi, '-').replace(/-+/g, '-').replace(/^-+|-+$/g, '');

    const pagePrefix = urlToFilename(pageUrl);
    return `${pagePrefix}-${name}${ext}`;
}
