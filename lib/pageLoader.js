...
export default async function pageLoader(pageUrl, outputDir = process.cwd()) {
    ...
    // Crear solo la carpeta de recursos
    ...
    $('img').toArray().forEach(img => {
        ...
        if (src && isLocalResource(src, pageUrl)) {
            resources.push({ el: img, attr: 'src', url: new URL(src, pageUrl).toString(), filename: urlToResourceName(src, pageUrl) });
        }
    });
    ...
    $('link[rel="stylesheet"]').toArray().forEach(link => {
        ...
        if (href && isLocalResource(href, pageUrl)) {
            resources.push({ el: link, attr: 'href', url: new URL(href, pageUrl).toString(), filename: urlToResourceName(href, pageUrl) });
        }
    });
    ...
    $('script[src]').toArray().forEach(script => {
        ...
        if (src && isLocalResource(src, pageUrl)) {
            resources.push({ el: script, attr: 'src', url: new URL(src, pageUrl).toString(), filename: urlToResourceName(src, pageUrl) });
        }
    });
