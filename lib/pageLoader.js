...
export default async function pageLoader(pageUrl, outputDir = process.cwd()) {
    ...
    const htmlFilePath = outputIsFile ? outputDir : path.join(outputDir, `${urlToFilename(pageUrl)}.html`);
    const resourcesDir = htmlFilePath.replace(/\.html$/, '_files');

    // Verificar que la carpeta base de htmlFilePath existe
    const baseDir = path.dirname(htmlFilePath);
    try {
        await fs.access(baseDir); // si no existe, esto lanzará un error
    } catch {
