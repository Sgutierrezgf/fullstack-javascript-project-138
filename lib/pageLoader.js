...
function urlToResourceName(resourceUrl, pageUrl) {
    ...
    const pagePath = new URL(pageUrl).pathname.replace(/^\/+|\/+$/g, ''); // "blog/about"
    let relPath = absoluteUrl.pathname.replace(/^\/+/, ''); // "blog/about/assets/styles.css"

    // Quitar el prefijo de la página si está presente
